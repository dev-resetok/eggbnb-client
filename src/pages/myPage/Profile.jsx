...
const Profile = ({info}) => {
    ...
    const thumbImg = (input) => {
        if(input.target.files && input.target.files[0]){
            ...
            imgRef.current.src = info.profileImg?`https://port-0-eggbnb-server-ly8jjkrt043c2434.sel5.cloudtype.app/${info.profileImg}`:`${publicUrl}/images/pages/detail/emptyUser.jpg`;
            setContent(<></>);
        }
    }
    ...
    const uploadFetch = async (formData) => {
        try{
            const response = await fetch("https://port-0-eggbnb-server-ly8jjkrt043c2434.sel5.cloudtype.app/user/myPage", {
                method : "POST",
                body : formData,
            })
                    ...
                    <label htmlFor="profileImg">
                        <div>
                            <img src={info.profileImg?
                                `https://port-0-eggbnb-server-ly8jjkrt043c2434.sel5.cloudtype.app/${info.profileImg}`
